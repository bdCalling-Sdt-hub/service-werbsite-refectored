...
export default function Page({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    try {
      ...
      fetch(apiUrl + "auth/register", {
        ...
        .then((json) => {
          if (json.ok) {
            router.push("/verify?id=" + json.data.id);
          } else {
            Swal.fire({
              icon: "error",
              text: json.message,
  ...
  return (
    <form
      className="max-w-screen-md rounded-2xl border-green-500 mx-auto border p-11 mt-20 grid grid-cols-2 gap-8"
    ...
    >
      ...
      <button
        type="submit"
        className="bg-green-500 p-3 text-white rounded-md col-span-2 font-light"
      >
        Sign Up
      </button>
